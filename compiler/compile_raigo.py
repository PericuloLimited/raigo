...
def compile_n8n(policy):
    ...
    # Each rule includes a full violation_response block so n8n Code nodes
    # can throw structured errors that appear in the execution log.
    policy_rules = []
    for rule in policies:
        vr = rule.get("violation_response", {})
        # Build a platform-specific override message if defined
        platform_msg = None
        for p in rule.get("platforms", []):
            if p.get("target") == "n8n":
                platform_msg = p.get("override_message")
                break
        ...
        policy_rules.append({
            ...
            "enforcement_message": platform_msg or rule.get("enforcement_message", ""),
            "compliance_refs": [
            ...
            "review_message": rule.get("review_message", ""),
            "tags": rule.get("tags", []),
            "violation_response": {
                "error_code": vr.get("error_code", f"RAIGO_{rule['action']}_{rule['id'].replace('-','')}"),
                "http_status": vr.get("http_status", 403 if rule["action"] == "DENY" else 422),
                "category": vr.get("category", rule.get("domain", "policy").lower().replace(" ", "_")),
                "user_message": vr.get("user_message", rule.get("enforcement_message", "Policy violation detected.")),
                "developer_message": vr.get("developer_message", f"Policy violation: Rule {rule['id']} ({rule.get('title', '')}). Compliance: {compliance_refs(rule)}."),
                "debug_hint": vr.get("debug_hint", ""),
                "audit_log": vr.get("audit_log", {"required": rule.get("audit_required", False), "fields": ["timestamp", "rule_id", "agent_id"]}),
                "next_steps": vr.get("next_steps", [])
            }
        })
...
def compile_claude(policy):
    ...
    for r in deny_rules:
        ...
        # Violation response block — enables structured error handling by the caller
        lines.append(f"        <violation_response>")
        lines.append(f"          <error_code>{vr.get('error_code', 'RAIGO_DENY_' + r['id'].replace('-',''))}</error_code>")
        lines.append(f"          <http_status>{vr.get('http_status', 403)}</http_status>")
        lines.append(f"          <user_message>{vr.get('user_message', r.get('enforcement_message', ''))}</user_message>")
        lines.append(f"          <developer_message>{vr.get('developer_message', '')}</developer_message>")
        lines.append(f"          <debug_hint>{vr.get('debug_hint', '')}</debug_hint>")
        lines.append(f"        </violation_response>")
        lines.append(f"      </rule>")
...
def compile_openclaw(policy):
    ...
    # violation_response is included so the OpenClaw gateway can throw
    # structured, catchable errors with developer_message and debug_hint.
    def build_violation_response(rule):
        vr = rule.get("violation_response", {})
        return {
            "error_code": vr.get("error_code", f"RAIGO_{rule['action']}_{rule['id'].replace('-','')}"),
            "http_status": vr.get("http_status", 403 if rule["action"] == "DENY" else 422),
    ...
    for rule in policies:
        gateway_rules.append({
            ...
            "tags": rule.get("tags", []),
            "violation_response": build_violation_response(rule)
        })
                ...
                "hard_deny": [
                    {
                        ...
                        "violation_response": build_violation_response(r)
                    }
                    for r in deny_rules
                ],
                "enforce_rules": [
