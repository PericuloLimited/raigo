...
// ─── Engine Call ─────────────────────────────────────────────────────────────
async function evaluateWithRaigo(prompt, channelId) {
    try {
        ...
        const result = (0, child_process_1.execSync)(`curl -s --max-time ${TIMEOUT_MS / 1000} -X POST ${ENGINE_URL}/v1/evaluate ` +
            `-H "Content-Type: application/json" ` +
            `-d '${payload.replace(/'/g, "'\\''")}'`, { timeout: TIMEOUT_MS + 500, encoding: "utf8" });
...
const handler = async (event) => {
    ...
    if (result === null) {
        if (!FAIL_OPEN) {
            event.messages.push("⚠️ **RAIGO Agent Firewall is not running.** " +
    ...
    if (result.action === "DENY") {
        ...
        const userMsg = result.user_message ??
            "This message was blocked by your organisation's AI security policy.";
        event.messages.push(`🛡️ **RAIGO Agent Firewall — Request Blocked**\n\n` +
            `${userMsg}\n\n` +
            `Rule: \`${ruleId}\` | ` +
            `[View Policy](https://raigo.ai/docs/openclaw)`);
        console.log(`[raigo-af] DENY — rule: ${ruleId}, channel: ${channelId}, ` +
    ...
    if (result.action === "WARN") {
        ...
        event.messages.push(`⚠️ **RAIGO Agent Firewall — Security Notice**\n\n` +
            `${userMsg}\n\n` +
            `Rule: \`${ruleId}\``);
        console.log(`[raigo-af] WARN — rule: ${ruleId}, channel: ${channelId}, ` +
            `session: ${event.sessionKey}, ts: ${event.timestamp.toISOString()}`);
        return;
    }
